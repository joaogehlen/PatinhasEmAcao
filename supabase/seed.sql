...
--
-- PRÉ-REQUISITO: as três contas de demonstração precisam existir no Auth antes
...
-- do servidor. Então crie as três pelo painel:
--
--   Authentication > Users > Add user > Create new user
--   Marque "Auto Confirm User" nas três.
--
...
-- 'morador'. Este script corrige os nomes e promove os dois primeiros.
--
...
-- os três perfis sem precisar de cadastro. Não use este seed se o app entrar em
-- uso real pela ONG.
-- =============================================================================
...
begin
  ...
  select id into volunteer_id from public.profiles where email = 'voluntario@patinhas.org';
  select id into resident_id  from public.profiles where email = 'morador@patinhas.org';

  if admin_id is null or volunteer_id is null or resident_id is null then
    raise exception
      'Contas de demonstração não encontradas. Crie as três em Authentication > Users (com Auto Confirm) antes de rodar este seed.';
  end if;

  update public.profiles set name = 'Administrador ONG', role = 'admin'      where id = admin_id;
  update public.profiles set name = 'Voluntária Ana',    role = 'voluntario' where id = volunteer_id;
  update public.profiles set name = 'Morador Carlos',    role = 'morador'    where id = resident_id;
  ...
  values
    ...
    ('Nina', 'gato', 'pequeno', 'femea', 30, 'docil',
     ...
     -28.8712, -52.1788, volunteer_id);

  raise notice 'Seed aplicado: 3 perfis ajustados e 7 animais criados.';
end $$;
